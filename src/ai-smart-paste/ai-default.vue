<template>
  <div class="control-pane">
    <div class="control-section">
      <form class="ai-form-container container bug-form-container" :style="formStyle">
        <div style="text-align: center; font-weight: 700;"><span>Bug Report Form</span></div>
        <div class="ai-single-row-group">
        ...
        <div class="ai-row-group">
          ...
          <div id="bug-priority">
            <label class="ai-form-label">Bug Priority</label>
            <div class="row">
        ...
        <div>
          <label for="browser" class="ai-form-label">Select the browser</label>
        ...
        <div class="ai-form-footer">
          ...
          <ejs-smartpaste id="smart-paste" cssClass="form-button" content="Smart Paste" :iconCss="'e-icons e-paste'"
            type="button" :aiAssistHandler="serverAIRequests" />
        </div>
      </form>
        ...
        <div class="e-card">
          <div class="e-card-content">
            ...
            <div class="chip-container">
              <ejs-chiplist id="chip-choice" :chips="bugPresets" selection="Single" :selectedChips="[0]"
                @click="chipsClickHandler" />
            </div>
            <div id="bug-report-text" v-html="selectedBugReport"></div>
            <ejs-button id="copy-btn" :content="copyButtonContent" :iconCss="copyButtonIcon"
              @click="onCopyClickHandler" />
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      copyButtonIcon: 'e-icons e-copy'
    };
  },

  mounted() {
    this.selectedBugReport = this.bugReports[0];
  },

  methods: {
    async serverAIRequests(options) {
      let output = '';
      try {
        output = await serverAIRequest(options);
      } catch (error) {
    ...
    async onCopyClickHandler() {
      await navigator.clipboard.writeText(this.selectedBugReport);
      this.copyButtonContent = "Copied";
      this.copyButtonIcon = "e-icons e-check";
    },

    chipsClickHandler(e) {
      this.selectedBugReport = this.bugReports[e.index];
      this.copyButtonContent = "Copy";
      this.copyButtonIcon = "e-icons e-copy";
    }
  }
};
</script>
