...
export let infiniteData: any[] = [];
export function infiniteDataSource(): void {
    let parent: number = -1;
    let parentId: number;
    let crew: string = 'Crew';
    let names: string[] = ['VINET', 'TOMSP', 'HANAR', 'VICTE', 'SUPRD', 'HANAR', 'CHOPS', 'RICSU', 'WELLI', 'HILAA', 'ERNSH', 'CENTC',
    ...
    for (let i: number = 0; i < 50000; i++) {
        ...
        if (i % 5 !== 0) {
            let num:number = isNaN((infiniteData.length % parent)- 1) ?  0 : (infiniteData.length % parent) - 1;

            infiniteData[num][crew].push({
                'TaskID': i + 1,
