...
export const employeeDetail: any[] = [
    {
        "EmployeeID": 'Emp1001',
        "Name": "John",
        "MailID": "john@abc.com",
...
export const data: Object[] = (<cType[]>orderData).map((item: cType) => {
...
let exportOrder: string = JSON.stringify([
    {
        'OrderID': 10248,
        'CustomerID': 'VINET',
        'CustomerName': 'Maria ',
...
let orderTemplateDetails: Object[] = JSON.parse(exportOrder, (field: string, value: any) => {
    let dValue: any = value;
    if (typeof value === 'string' && /^(\d{4}\-\d\d\-\d\d([tT][\d:\.]*){1})([zZ]|([+\-])(\d\d):?(\d\d))?$/.test(value)) {
        let array: any = dValue.split(/[^0-9]/);
        value = new Date(parseInt(array[0], 10), parseInt(array[1], 10) - 1,
            parseInt(array[2], 10), parseInt(array[3], 10), parseInt(array[4], 10), parseInt(array[5], 10));
    }
    return value;
});
...
templateDetails.forEach(function(e: any) {
...
export const data1: Object[] = (<cType[]>orderDataSource).map((item1: cType) => {
    let name1: cType = (<cType[]>customerData).filter((cItem1: cType) => {
        return cItem1.CustomerID === item1.CustomerID;
    })[0];
    item1.CustomerName = (name1 || <cType>{}).ContactName;
    return item1;
});
