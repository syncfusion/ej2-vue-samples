<template>
  <div>
    ...
    <div id="action-description">
      <p>
        This sample demonstrates rendering Heatmap control by providing desired data from a pivot grid on selection. Not
    ...
    <div id="description">
      <p>
        ...
        <code>cellSelected</code> event event gets fired on every selection
...
export default Vue.extend({
  ...
  methods: {
    dataBound: function() {
      let pivotGridObj = (<any>this.$refs.pivotview_heatmap).ej2Instances;
      if (init) {
    ...
    heatmapUpdate: function() {      
      ...
      heatmap.xAxis = {
        title: { text: pivotGridObj.dataSource.rows.map(function (args:any) { return args.caption || args.name; }).join(' ~ ') },
        labels: xLabels,
        labelRotation: 315
      };
      heatmap.yAxis = {
        title: { text: pivotGridObj.dataSource.values.map(function (args:any) { return args.caption || args.name; }).join(' ~ ') },
        labels: yLabels
      };
      heatmap.refresh();
    },
