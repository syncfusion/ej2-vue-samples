<template>
  <div class="control-section e-tab-section">
   <div class="col-lg-8 control-section">
    <div class="e-sample-resize-container">
        <ejs-toolbar id='toolbar_rtl' enableRtl='true'>
            <e-items>
                <e-item prefixIcon='e-cut-icon tb-icons' tooltipText='Cut' overflow='Show'></e-item>
                <e-item prefixIcon='e-copy-icon tb-icons' tooltipText='Copy' overflow='Show'></e-item>
                <e-item prefixIcon='e-paste-icon tb-icons' tooltipText='Paste' overflow='Show'></e-item>
                <e-item type='Separator'></e-item>
                <e-item prefixIcon='e-bold-icon tb-icons' tooltipText='Bold' overflow='Show'></e-item>
                <e-item prefixIcon='e-underline-icon tb-icons' tooltipText='Underline' overflow='Show'></e-item>
                <e-item prefixIcon='e-italic-icon tb-icons' tooltipText='Italic' overflow='Show'></e-item>
                <e-item type='Separator'></e-item>
                <e-item prefixIcon='e-bullets-icon tb-icons' tooltipText='Bullets' text='Bullets' showTextOn='Overflow'></e-item>
                <e-item prefixIcon='e-numbering-icon tb-icons' tooltipText='Numbering' text='Numbering' showTextOn='Overflow'></e-item>
                <e-item type='Separator'></e-item>
                <e-item prefixIcon='e-alignleft-icon tb-icons' tooltipText='Align-Left' showTextOn='Overflow' text='Left' overflow='Show'></e-item>
                <e-item prefixIcon='e-alignright-icon tb-icons' tooltipText='Align-Right' showTextOn='Overflow' text='Right'></e-item>
                <e-item prefixIcon='e-aligncenter-icon tb-icons' tooltipText='Align-Center' showTextOn='Overflow' text='Center'></e-item>
                <e-item prefixIcon='e-alignjustify-icon tb-icons' tooltipText='Align-Justify' showTextOn='Overflow' overflow='Show' text='Justify'></e-item>.
                <e-item type='Separator'></e-item>
                <e-item prefixIcon='e-undo-icon tb-icons' tooltipText='Undo' text='Undo'></e-item>
                <e-item prefixIcon='e-redo-icon tb-icons' tooltipText='Redo' text='Redo'></e-item>
                <e-item type='Separator'></e-item>
                <e-item prefixIcon='e-radar-icon tb-icons' tooltipText='Radar Chart' text='Radar'></e-item>
                <e-item prefixIcon='e-line-icon tb-icons' tooltipText='Line Chart' text='Line'></e-item>
                <e-item type='Separator'></e-item>
                <e-item prefixIcon='e-table-icon tb-icons' tooltipText='Table' showTextOn='Overflow' text='Table'></e-item>
                <e-item prefixIcon='e-picture-icon tb-icons' tooltipText='Picture' showTextOn='Overflow' overflow='Hide' text='Picture'></e-item>
                <e-item prefixIcon='e-design-icon tb-icons' tooltipText='Design' showTextOn='Overflow' overflow='Hide' text='Design'></e-item>
            </e-items>
        </ejs-toolbar>
        <div id="action-description">
        <p>This sample demonstrates the RTL mode of the <code>Toolbar</code>. Select the option in property panel for switching mode in Toolbar with right to left direction.</p>
    </div>

    <div id="description">
        <p>The RTL sample illustrates the direction of the toolbar from <strong>right to left</strong>. You can switch between <strong>Popup</strong> and <strong>Scrollable</strong> mode using the options given in the drop down list.</p>
    </div>
    </div>
    </div>
<div class="col-lg-4 property-section">
                <table id="property" title="Toolbar Mode Switch in RTL">
                    <tr>
                        <td style="width:30%">Mode</td>
                        <td style="width:70%;padding-right: 10px">
                              <ejs-dropdownlist id='drop' width='90%' :dataSource='dataPlace' :change='placeChange' :value='placeValue' :fields='placeFields'>
                            </ejs-dropdownlist>
                        </td>
                    </tr>
                </table>
            </div>
</div>
    
</template>
<style>
@font-face {
    font-family: 'Material_toolbar_right';
    src: url(data:application/x-font-ttf;charset=utf-8;base64,) format('truetype');
    font-weight: normal;
    font-style: normal;
}

.e-tbar-btn .tb-icons {
    font-family: 'Material_toolbar_right';
    speak: none;
    font-size: 16px;
    font-style: normal;
    font-weight: normal;
    font-variant: normal;
    text-transform: none;
}

.e-bigger .e-tbar-btn .tb-icons {
    font-size: 18px;
}

.e-tbar-menu-icon:before {
    content: "\e718";
}

.e-tbar-search-icon:before {
    content: "\e71d";
}

.e-tbar-settings-icon:before {
    content: "\e702";
}

.e-tbar-section .e-toolbar .e-toolbar-item.e-template {
    cursor: default;
}

.e-def-avator {
    margin: 8px;
    border-radius: 50%;
    width: 40px;
    font-size: 20px;
    border: 2px solid #666;
}

.e-def-avator span {
    text-align: center;
    line-height: 40px;
    display: block;
}

.e-mail-header,
.e-mail-subject,
.e-mail-description,
.e-mail-id {
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
}

.e-folder {
    text-align: center;
}

.e-mail-header,
.e-mail-subject,
.e-folder {
    font-weight: 500;
    font-style: normal;
}

.e-mail-item {
    margin-top: 8px;
    border-bottom: 1px solid #d3d3d3;
    height: 100%;
    overflow: hidden;
}

.e-mail-image {
    margin-right: 10px;
    float: left;
    height: 60px;
}

.e-cut-icon:before {
    content: "\e719"
}

.e-copy-icon:before {
    content: "\e721"
}

.e-paste-icon:before {
    content: "\e712"
}

.e-color-icon:before {
    content: "\e703";
}

.e-bold-icon:before {
    content: "\e71a"
}

.e-underline-icon:before {
    content: "\e706";
}

.e-alignleft-icon:before {
    content: "\e717"
}

.e-alignright-icon:before {
    content: "\e715"
}

.e-aligncenter-icon:before {
    content: "\e704"
}

.e-alignjustify-icon:before {
    content: "\e71b"
}

.e-upload-icon:before {
    content: "\e71e"
}

.e-download-icon:before {
    content: "\e70a"
}

.e-indent-icon:before {
    content: "\e70b"
}

.e-outdent-icon:before {
    content: "\e700"
}

.e-clear-icon:before {
    content: "\e70d"
}

.e-reload-icon:before {
    content: "\e71c"
}

.e-export-icon:before {
    content: "\e720";
}

.e-italic-icon:before {
    content: "\e710"
}

.e-bullets-icon:before {
    content: "\e711";
}

.e-numbering-icon:before {
    content: "\e70e";
}

.e-ascending-icon:before {
    content: "\e70f";
}

.e-descending-icon:before {
    content: "\e707";
}

.e-design-icon:before {
    content: '\e71f'
}

.e-table-icon:before {
    content: '\e716'
}

.e-picture-icon:before {
    content: '\e701'
}

.e-tbar-redo-icon:before {
    content: "\e713"
}

.e-doughnut-icon:before {
    content: "\e70c"
}

.e-bubble-icon:before {
    content: "\e709"
}

.e-line-icon:before {
    content: "\e705"
}

.e-radar-icon:before {
    content: "\e714"
}

.e-tbar-undo-icon:before {
    content: "\e708"
}

.e-tool-name {
    font-style: normal;
    font-size: 16px;
    cursor: default;
}

.e-icon-week:before {
  content: '\e951';
}

.e-print:before {
  content: '\e813';
}

.e-icon-month:before {
  content: '\e93c';
}

.e-icon-year:before {
  content: '\e812';
}

.e-reccurence-icon:before {
  content: '\e80d';
  font-weight: 700;
}

.e-icon-day:before {
  content: '\e970';
}

@media only screen and (min-width: 1224px) {
    .e-tbar-section .e-sample-resize-container {
        width: 700px;
    }
}

@media only screen and (min-width: 1824px) {
    .e-tbar-section .e-sample-resize-container {
        width: 50%;
    }
}

.e-tbar-section .e-sample-resize-container {
    margin: auto;
    height: 40px;
    max-width: 700px;
}

.material .e-bigger .e-toolbar .e-tbar-btn .e-icons {
    font-size: 18px;
}

.material .e-toolbar .e-tbar-btn .e-icons {
    font-size: 16px;
}
</style>
<script>
import Vue from "vue";
import { ToolbarPlugin } from "@syncfusion/ej2-vue-navigations";
Vue.use(ToolbarPlugin);
export default Vue.extend({
data: function(){
        return {
            dataPlace: [{ id: 'scrollable', mode: 'Scrollable' }, { id: 'popup', mode: 'Popup' }],
            placeValue: 'scrollable',
            placeFields: { text: 'mode', value: 'id' },
    }
   },
     methods: {
       placeChange: function(args) {
           let tabObj=document.getElementById('toolbar_rtl');
           if (args.value === 'scrollable') {
               tabObj.ej2_instances[0].overflowMode = 'Scrollable';
            } else {
              tabObj.ej2_instances[0].overflowMode = 'Popup';
           }
           tabObj.ej2_instances[0].dataBind();
       }
    }
});
</script>