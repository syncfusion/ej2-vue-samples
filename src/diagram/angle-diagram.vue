<template>
  <div class="control-section angle-diagram-container" style="width: 100%">
    <ejs-diagram id="diagram" ref="diagramObj" :width="width" :height="height" :nodes="nodes" :connectors="connectors"
      :snapSettings="snapSettings" :selectedItems="selectedItems" :pageSettings="pageSettings"
      @rotateChange="onRotationChange" @created="created" @click="diagramClick" @load="load"
  ...
  <div id="description">
    <p>
      ...
      users can adjust the panel's tilt angle either by direct rotation or using the numeric input. The system provides
      immediate feedback through a dynamic efficiency gauge and performance messages. Users can also select different
      locations to observe how regional variations impact overall solar efficiency.
    </p>
  </div>
</template>
...
function createNativeNode(id, width, height, offsetX, offsetY, svgContent, constraints = NodeConstraints.None) {
  return {
    id, width, height, offsetX, offsetY,
    constraints,
...
function createEllipseNode(id, width, height, offsetX, offsetY, fill, strokeColor, strokeWidth, constraints = NodeConstraints.None) {
  return {
    id, width, height, offsetX, offsetY,
    constraints,
...
function createHtmlNode(id, width, height, offsetX, offsetY, html, constraints = NodeConstraints.None) {
  return {
    id, width, height, offsetX, offsetY,
    constraints,
...
function createRectNode(id, width, height, offsetX, offsetY, fill, strokeColor, strokeWidth, constraints = NodeConstraints.None) {
  return {
    id, width, height, offsetX, offsetY,
    constraints,
    style: { fill, strokeColor, strokeWidth }
...
function getAngleHtml() {
  // Keep initial text same as original: "Medium Tilt"
  return `
    <div class="angle-control-section" style="width: 300px; height: 185px;">
      ...
      <div class="angle-description" id="angleDescription">Medium Tilt</div>
    </div>`;
}
function createNodes(solarData) {
  return [
    // Title
    createTextNode(
      'title',
    ...
    // East sun
    createNativeNode('eastSun', 60, 60, 221, 422, eastSunSvg, NodeConstraints.None),
    // Center sun
    createNativeNode('centerSun', 60, 60, 483, 293, centerSunSvg, NodeConstraints.None),
    // West sun
    createNativeNode('westSun', 60, 45, 731, 422, westSunSvg, NodeConstraints.None),
    // Labels
    createTextNode(
      'eastLabel',
      60,
    ...
    // Location selector
    createHtmlNode('location', 300, 150, 1130, 100, getLocationHtml(), NodeConstraints.None),
    // Efficiency
    createHtmlNode('efficiency', 300, 350, 1130, 383, getEfficiencyHtml(), NodeConstraints.None),
    // Angle input
    createHtmlNode('angle', 300, 185, 1130, 680, getAngleHtml(), NodeConstraints.None)
  ];
}
...
export default {
  ...
  data: function () {
    ...
    const solarData = {
      currentAngle: 303,       // diagram rotateAngle (0..360)
      efficiency: 78,          // display percentage
      ...
      solarIrradiance: 0,
      incidenceAngle: 0,
      intPanelAngleDeg: 0      // internal 0..100 mapping of currentAngle
    };

    const nodes = createNodes(solarData);
    ...
    return {
      ...
      // new for NumericTextBox control
      angleNumeric: null,
      suppressAngleChange: false,
      lastValidRelativeAngle: 0
    };
  },
  methods: {
    // Restrict rotation to [303..360] and [0..44]
    onRotationChange(args) {
      if (args.state === 'Completed') {
        if (args.source && args.source.nodes && args.source.nodes[0]) {
          const node = args.source.nodes[0];
          if (node.id === 'solarPanelFrame') {
            this.solarData.currentAngle = parseInt(args.newValue.rotateAngle.toString());
        ...
        if (args.source && args.source.nodes && args.source.nodes[0]) {
          const selectedNode = args.source.nodes[0];
          if (selectedNode.id === 'solarPanelFrame') {
            ...
            if (
              !(
                (normalizedAngle >= 303 && normalizedAngle <= 360) ||
    ...
    scrollChange() {
      if (this.locationDropdown?.isPopupOpen) {
    ...
    created() {
      ...
      this.diagram = this.$refs.diagramObj.ej2Instances;
    ...
    diagramClick() {
      ...
      const solarPanelNode = this.diagram.getObject('solarPanelFrame');
      if (solarPanelNode) this.diagram.select([solarPanelNode]);
    },

    // Re-initialize controls when the diagram content is reloaded
    load() {
      setTimeout(() => {
        if (this.diagramCreated && this.$refs.diagramObj.ej2Instances) {
          this.initializeLocationDropdown();
          this.initializeEfficiencyGauge();
          this.initializePerformanceMessage();
          this.initializeAngleControls();
    ...
    onLocationChanged(args) {
      ...
      const loc = this.getLocationData(this.solarData.selectedLocation);
      if (loc) {
        // map 0..100 to diagram allowed range 303..360, 0..43
        const locationAngle = Math.min(100, loc.angle);
        if (locationAngle <= 57) {
          this.solarData.currentAngle = 303 + locationAngle; // 0..57 -> 303..360
        } else {
        ...
        axes: [
          {
            ...
            pointers: [
              {
                type: 'Marker',
                value: this.solarData.efficiency,
    ...
    // New: NumericTextBox-based angle control
    initializeAngleControls() {
      // Destroy previous instance if any
      if (this.angleNumeric) {
        this.angleNumeric.destroy();
        this.angleNumeric = null;
      }

      const relativeAngle = this.solarData.intPanelAngleDeg - 57; // -57..43
      this.lastValidRelativeAngle = relativeAngle;
      ...
      this.angleNumeric = new NumericTextBox({
        ...
        change: (args) => {
          ...
          const val =
            typeof args.value === 'number' ? args.value : this.angleNumeric.value;
      ...
      // Prevent diagram interaction when editing input
      ...
      if (inputEl) {
        ...
        inputEl.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') {
            e.preventDefault();
            inputEl.blur(); // commit & clamp with strictMode
      ...
      // Shortcut: if user types numeric keys while diagram focused, focus the input
      // Note: keep a single listener; guard re-adding if needed.
      if (!this._docKeydownAttached) {
        document.addEventListener(
          'keydown',
          (e) => {
            const input = document.getElementById('angleValue');
            const active = document.activeElement;
            if (active && (active === input || active?.closest('.e-input-group') === input?.closest('.e-input-group'))) return;
        ...
        this._docKeydownAttached = true;
      }
    },

    // Convert relative angle (-57..43) to diagram rotateAngle [303..360,0..43]
    ...
    // Compute intPanelAngleDeg 0..100 from currentAngle
    angleCalculation() {
      const a = this.solarData.currentAngle;
      if (a >= 303 && a <= 360) {
        // 303..360 -> 0..57
        this.solarData.intPanelAngleDeg = a - 303;
      } else if (a >= 0 && a <= 43) {
        // 0..43 -> 57..100
        this.solarData.intPanelAngleDeg = 57 + a;
      } else {
        // keep within [0,100] if out of band
        this.solarData.intPanelAngleDeg = Math.max(
          0,
          Math.min(100, this.solarData.intPanelAngleDeg)
        );
      }
    },
    ...
    calculateSolarPosition() {
      ...
      const selectedDate = new Date(this.solarData.selectedDateTime);
      ...
      const declination =
        ((23.45 * Math.PI) / 180) * Math.sin((2 * Math.PI * (284 + dayOfYear)) / 365);
      const hourAngle = 0; // noon
      const elevation = Math.asin(
        Math.sin(declination) * Math.sin(lat) +
        Math.cos(declination) * Math.cos(lat) * Math.cos(hourAngle)
      );

      this.solarData.sunElevation = Math.max(0, (elevation * 180) / Math.PI);
      ...
      if (this.solarData.sunElevation > 0) {
        ...
        this.solarData.solarIrradiance =
          1353 * Math.pow(0.7, Math.pow(airMass, 0.678)) * Math.sin(elevationRad);
    ...
    updateDiagram() {
      if (this.diagram) {
        const panel = this.diagram.getObject('solarPanelFrame');
        if (panel) {
          panel.rotateAngle = this.solarData.currentAngle;
          this.diagram.dataBind();
        }
      }
    },

    // Keep NumericTextBox synchronized with model without recursive events
    syncAngleInputFromModel() {
      if (!this.angleNumeric) return;
      const relative = this.solarData.intPanelAngleDeg - 57;
      this.suppressAngleChange = true;
      this.angleNumeric.value = relative;
      this.angleNumeric.dataBind();
      this.suppressAngleChange = false;
      this.lastValidRelativeAngle = relative;
    },

    updateUI() {
      // Sync numeric input from model
      this.syncAngleInputFromModel();

      // Angle description
      const descEl = document.getElementById('angleDescription');
      if (descEl) {
        descEl.textContent = this.getAngleDescription();
      }
      ...
      if (this.performanceMessage) {
        const msg = this.getPerformanceMessage();
        this.performanceMessage.content = msg.message;
        this.performanceMessage.severity = msg.severity;
    ...
    getAngleDescription() {
      // relative 0 (intPanelAngleDeg==57) is “Horizontal”
    ...
    getPerformanceMessage() {
      if (this.solarData.efficiency > 60) {
        return { message: 'Excellent Performance', severity: Severity.Success };
      } else if (this.solarData.efficiency > 40) {
        return { message: 'Fair Performance', severity: Severity.Warning };
      } else {
    ...
    getLocationData(name) {
      return locationData.find((loc) => loc.name.toLowerCase() === name.toLowerCase());
    }
  }
};
</script>
...
<style>
.angle-diagram-container .e-ddl.e-input-group .e-input-group-icon,
