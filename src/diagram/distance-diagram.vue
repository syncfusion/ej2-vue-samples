...
.diagram-distance-measurement .sb-mobile-diagram {
  width: 100%;
  height: 750px;
  float: left;
}
...
export default {
  ...
  methods: {
    ...
    onSourcePointChanged(args) {
        this.updateDimensionConnectors(args.connector);
    },

    onTargetPointChanged(args) {
        this.updateDimensionConnectors(args.connector);
    },

    onUnitChanged(args) {
        this.selectedUnit = args.value;
        this.updateConnectorAnnotationsFromStoredDistances();
    },

    createWallConnector(id, startX, startY, endX, endY, color, thickness) {
      let middlePortPosition = 0.5;
    ...
    createItem(id, x, y, width, height, pathData, options) {
      let item = {
        id: id,
        width: width,
        height: height,
    ...
    createBedroomItem(id, x, y, width, height, pathData, color) {
      let options = {
        fill: '#E8F4FD',
        strokeColor: '#2E86AB',
      };
    ...
    createKitchenItem(id, x, y, width, height, pathData, color) {
      let options = {
        fill: color,
        strokeColor: '#333',
      };
    ...
    createLivingRoomItem(id, x, y, width, height, pathData, color) {
      let options = {
        fill: '#FFF8DC',
        strokeColor: '#B8860B',
      };
    ...
    createBathroomDoor(id, x, y, width, height, pathData, color) {
        let options = {
            fill: '#E6E6FA',
            strokeColor: '#9370DB',
            rotateAngle: 270,
    ...
    createDimensionConnector(id, startX, startY, endX, endY, dimensionText, isHorizontal) {
      let dimensionLine = {
        id: id,
        sourcePoint: { x: startX, y: startY },
        targetPoint: { x: endX, y: endY },
      ...
      if (isHorizontal) {
        this.createExtensionLine(id + '_ExtStart', startX, startY - 10, startX, startY + 10);
        this.createExtensionLine(id + '_ExtEnd', endX, endY - 10, endX, endY + 10);
      } else {
        this.createExtensionLine(id + '_ExtStart', startX - 10, startY, startX + 10, startY);
        this.createExtensionLine(id + '_ExtEnd', endX - 10, endY, endX + 10, endY);
      }
    },

    createExtensionLine(id, startX, startY, endX, endY) {
      let extensionLine = {
        id: id,
        sourcePoint: { x: startX, y: startY },
        targetPoint: { x: endX, y: endY },
    ...
    createInitialLayout() {
      ...
      // Update diagram if it's available
      if (this.diagram) {
        this.diagram.dataBind();
      }
    },

    addMeasurementConnectorsRuntime() {
    ...
    addOverallMeasurementConnectors() {
      let wallOuterTop = this.connectors.find((c) => c.id === 'WallOuterTop');
      let wallOuterLeft = this.connectors.find((c) => c.id === 'WallOuterLeft');
      let wallOuterBottom = this.connectors.find((c) => c.id === 'WallOuterBottom');
      let wallOuterRight = this.connectors.find((c) => c.id === 'WallOuterRight');
      ...
      if (wallOuterTop && wallOuterLeft && wallOuterBottom && wallOuterRight) {
        ...
        this.createDimensionConnector(
          'TotalWidthMeasurement',
          wallOuterLeft.sourcePoint.x,
          wallOuterTop.sourcePoint.y - 50,
          wallOuterRight.targetPoint.x,
          wallOuterTop.sourcePoint.y - 50,
          widthText,
          true
        );
        ...
        this.createDimensionConnector(
          'TotalHeightMeasurement',
          wallOuterLeft.sourcePoint.x - 70,
          wallOuterTop.sourcePoint.y,
          wallOuterLeft.sourcePoint.x - 70,
          wallOuterBottom.targetPoint.y,
          heightText,
          false
        );
      }
    ...
    addSpecificSeparatorMeasurement(separator) {
      let wallOuterTop = this.connectors.find((c) => c.id === 'WallOuterTop');
      let wallOuterBottom = this.connectors.find((c) => c.id === 'WallOuterBottom');
      let wallOuterRight = this.connectors.find((c) => c.id === 'WallOuterRight');
      let yOffset, xOffset, distance;
      ...
      switch (separator.id) {
        case 'Separator1':
          ...
          this.createDimensionConnector(
            separator.id + 'Measurement',
            separator.sourcePoint.x, yOffset, separator.targetPoint.x, yOffset,
            this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), true
          );
          break;
        case 'Separator3':
          distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x);
        ...
        case 'HallSeparator':
          ...
          this.createDimensionConnector(
            separator.id + 'Measurement',
            xOffset, separator.sourcePoint.y, xOffset, separator.targetPoint.y,
            this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), false
          );
          break;
        case 'Separator4':
          distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x);
    ...
    convertPixelsToSelectedUnit(pixelValue) {
    ...
    updateConnectorAnnotationsFromStoredDistances() {
      if (!this.diagram || !this.diagram.connectors) return;
      ...
      this.diagram.connectors.forEach((connector) => {
        if (
          ...
          let content = connector.annotations[0].content;
          ...
          if (parts.length > 1) {
            ...
            connector.annotations[0].content = this.formatMeasurement(convertedValue);
          }
        }
      });
    ...
    updateDimensionConnectors(movedConnector) {
      if (!this.diagram) return;
      
      let id = movedConnector.id;
      if (
        ...
        let dimConnector = this.diagram.getObject(measurementId);
        let extStartConnector = this.diagram.getObject(measurementId + '_ExtStart');
        let extEndConnector = this.diagram.getObject(measurementId + '_ExtEnd');

        if (!dimConnector || !extStartConnector || !extEndConnector) return;

        let wallOuterTop = this.diagram.getObject('WallOuterTop');
        let wallOuterBottom = this.diagram.getObject('WallOuterBottom');
        let wallOuterRight = this.diagram.getObject('WallOuterRight');
        let startX, startY, endX, endY, distance, isHorizontal;

        switch (id) {
          case 'Separator1':
            distance = Math.abs(movedConnector.targetPoint.x - movedConnector.sourcePoint.x);
            startX = movedConnector.sourcePoint.x; endX = movedConnector.targetPoint.x;
            startY = endY = wallOuterBottom.sourcePoint.y + 40; isHorizontal = true;
            break;
          case 'Separator3':
            distance = Math.abs(movedConnector.targetPoint.x - movedConnector.sourcePoint.x);
            startX = movedConnector.sourcePoint.x; endX = movedConnector.targetPoint.x;
            startY = endY = wallOuterBottom.sourcePoint.y + 20; isHorizontal = true;
            break;
          case 'HallSeparator':
            distance = Math.abs(movedConnector.targetPoint.y - movedConnector.sourcePoint.y);
            startY = movedConnector.sourcePoint.y; endY = movedConnector.targetPoint.y;
            startX = endX = 270; isHorizontal = false;
            break;
          case 'Separator4':
            distance = Math.abs(movedConnector.targetPoint.x - movedConnector.sourcePoint.x);
            startX = movedConnector.sourcePoint.x; endX = movedConnector.targetPoint.x;
            startY = endY = wallOuterTop.sourcePoint.y - 20; isHorizontal = true;
            break;
          case 'Separator2':
            distance = Math.abs(movedConnector.targetPoint.y - movedConnector.sourcePoint.y);
            startY = movedConnector.sourcePoint.y; endY = movedConnector.targetPoint.y;
            startX = endX = wallOuterRight.sourcePoint.x + 30; isHorizontal = false;
            break;
          case 'RestroomSeparator':
            distance = Math.abs(movedConnector.targetPoint.y - movedConnector.sourcePoint.y);
            startY = movedConnector.sourcePoint.y; endY = movedConnector.targetPoint.y;
            startX = endX = wallOuterRight.sourcePoint.x + 60; isHorizontal = false;
            break;
          default:
            return;
        }
        ...
        dimConnector.annotations[0].content = this.formatMeasurement(
