  ...
  <div>
    <div class="col-lg-9 control-section">
      <div class="default-section">
        ...
        <ejs-documenteditorcontainer ref="doceditcontainer" :serviceUrl="hostUrl" :documentEditorSettings="settings"
          :enableToolbar='true' height='600px'></ejs-documenteditorcontainer>
      </div>
    </div>
    <div class="col-lg-3 property-section">
      <div class="content-wrapper sb-content-tab-header">
      <div class="property-panel-header">Form UI</div>
        <div>
          <input id="FirstName" />
        </div>
      ...
      <div>
        <ejs-button style="margin: 20px 10px 10px 0px;" id='BindToDocument' v-on:click="bindToDocumentBtnClick">Bind
          Form UI Data To Document</ejs-button>
        <ejs-button style="margin: 10px 0px; white-space: normal;" id='BindToForm'
          v-on:click="bindToFormBtnClick">Bind Document Data To Form UI</ejs-button>
      </div>
    </div>
    <div id="action-description">
      <p>This example showcases how to retrieve content control data from a document and subsequently import data into
        it.</p>
    </div>
    <div id="description">
      <p>Content controls are individual controls that users can add and customize for use in templates, forms, and
        documents.</p>
      <ul>
        ...
        <li>Dropdown List: Create and customize dropdown menus to streamline user input and enhance form
          functionality.
        ...
        <li>Check Box: Add check boxes for easy task tracking and selection, perfect for creating lists and forms.
          Users
      ...
      <p style="display: block">More information about the document editor content control can be found in this
        <a target="_blank"
          href="https://ej2.syncfusion.com/vue/documentation/document-editor/content-control/">documentation section.</a>
      </p>
    </div>
  </div>
</template>
...
//import { DocumentEditorContainerComponent, Toolbar, ContentControlInfo } from '@syncfusion/ej2-documenteditor';
import { DropDownButtonComponent } from "@syncfusion/ej2-vue-splitbuttons";
import { binduitodocument } from "./data";
...
export default {
  ...
  data: function () {
    return {
      ...
      documentName: 'Bind Content Control Data',
  ...
  methods: {
    ...
    bindDataToFormUI: function (isNeedFocus) {
      let contentControlInfos = this.$refs.doceditcontainer.ej2Instances.documentEditor.exportContentControlData();
      if(contentControlInfos.length > 0) {
      for (let i = 0; i < contentControlInfos.length; i++) {
        if (!this.isNullOrUndefined(contentControlInfos[i].title) && contentControlInfos[i].title.indexOf('Name') > -1) {
          document.getElementById('FirstName').value = contentControlInfos[i].value;
        }
        if (!this.isNullOrUndefined(contentControlInfos[i].title) && contentControlInfos[i].title.indexOf('DOB') > -1) {
          document.getElementById('BirthDate').value = contentControlInfos[i].value;
        }
        if (!this.isNullOrUndefined(contentControlInfos[i].title) && contentControlInfos[i].title.indexOf('Address') > -1) {
          document.getElementById('Address').value = contentControlInfos[i].value;
        }
        if (!this.isNullOrUndefined(contentControlInfos[i].title) && contentControlInfos[i].title.indexOf('Phone') > -1) {
          document.getElementById('Phone').value = contentControlInfos[i].value;
        }
        if (!this.isNullOrUndefined(contentControlInfos[i].title) && contentControlInfos[i].title.indexOf('Email') > -1) {
          document.getElementById('Email').value = contentControlInfos[i].value;
        }
      }
      }
      if (isNeedFocus) {
    ...
    bindToFormBtnClick: function () {
      this.bindDataToFormUI(false);
    },
    ...
    bindToDocumentBtnClick: function () {
      let bookmarkobj = {};
      const data = [];
      bookmarkobj['Name'] = document.getElementById('FirstName').value;
      bookmarkobj['DOB'] = document.getElementById('BirthDate').value;
      bookmarkobj['Address'] = document.getElementById('Address').value;
      bookmarkobj['Phone'] = document.getElementById('Phone').value;
      bookmarkobj['Email'] = document.getElementById('Email').value;
      if (!this.isNullOrUndefined(bookmarkobj['Name'])) {
      ...
      if (!this.isNullOrUndefined(bookmarkobj['DOB'])) {
      ...
      if (!this.isNullOrUndefined(bookmarkobj['Address'])) {
      ...
      if (!this.isNullOrUndefined(bookmarkobj['Phone'])) {
      ...
      if (!this.isNullOrUndefined(bookmarkobj['Email'])) {
      ...
      this.$refs.doceditcontainer.ej2Instances.documentEditor.importContentControlData(data);
    },
  ...
  mounted() {
    this.$nextTick(function () {
      ...
      let firstNameTextBox = new TextBox({
        placeholder: 'Name',
        cssClass: 'e-outline',
        floatLabelType: 'Auto'
      });
      ...
      let addressTextBox = new TextBox({
        placeholder: 'Address',
        cssClass: 'e-outline',
        floatLabelType: 'Auto'
      });
      ...
      editor.open(JSON.stringify(binduitodocument));
      editor.documentName = 'Bind Content Control Data';
      // To bind the data to form UI
      this.bindDataToFormUI(true);
