...
<div>
    ...
    <div id="action-description">
      <p>This sample demonstrates the chunk upload functionalities of the Uploader component. Browse the files which you want to upload to the server. 
         The selected files are submitted to server on upload button click based on the selected chunk size. Also, You can pause the uploading file and resuming it.
         Once the upload got failed, it will retry automatically. Even it fails to upload in auto retry, retry button will be enabled to retry the upload later.</p>
    </div>
    ...
    <div id="description">
        <p>When the file size is large or transfer the file with slow network connection, the chunk upload feature slices the files and upload the sliced chunks to server in sequential order
           using the <a href="https://ej2.syncfusion.com/vue/documentation/uploader/api-asyncSettingsModel.html#chunksize"
          ...
          <p>More information on the Uploader instantiation can be found in this <a target="_blank" href="https://ej2.syncfusion.com/vue/documentation/uploader/chunk-upload.html">documentation section</a>.</p>
    </div>
</div>
</template>
