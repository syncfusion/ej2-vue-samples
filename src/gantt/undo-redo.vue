<template>
  <div class="col-lg-12 control-section">
    <div id="action-description">
      <p>This sample showcases the undo-redo functionality in the Gantt Chart, allowing users to revert or reapply their recent actions
        such as task edits, additions, and deletions.</p>
    </div>
    <div>
      <ejs-gantt ref="gantt" id="GanttUndoRedo" :dataSource="data" :height="height" :rowHeight="46" :taskbarHeight="25"
        :taskFields="taskFields" :allowSelection="true" :allowFiltering="true" :allowSorting="true" :allowResizing="true"
        :allowReordering="true" :enableContextMenu="true" :treeColumnIndex="1" :showColumnMenu="true" :splitterSettings="splitterSettings"
        :labelSettings="labelSettings" :projectStartDate="projectStartDate" :projectEndDate="projectEndDate" :highlightWeekends="true"
        :enableUndoRedo="true" :undoRedoActions="undoRedoActions" :toolbar="toolbar" :editSettings="editSettings" :columns="columns"
        :dataBound="dataBound" :toolbarClick="toolbarClick" :actionComplete="actionComplete" :resizeStop="resizeStop"></ejs-gantt>
    </div>
    <div id="description">
      <p>The undo feature lets users reverse the most recent changes made to tasks, dependencies, columns, or timeline settings. The redo feature restores actions that were previously undone. Both can be triggered using keyboard shortcuts (<code>Ctrl + Z</code> for Undo, <code>Ctrl + Y</code> for Redo on Windows/Linux or <code>Cmd + Z</code> / <code>Cmd + Shift + Z</code> on Mac) or via toolbar buttons.</p>
      <p>
      ...
      <p>
        Gantt component features are segregated into individual feature-wise modules. To use undo redo, filter, sort, column menu, resize, edit, reorder, context menu, toolbar, markers features, inject the
        <code>UndoRedo</code>, <code>Filter</code>, <code>Sort</code>, <code>ColumnMenu</code>, <code>Resize</code>, <code>Edit</code>, <code>Reorder</code>, <code>ContextMenu</code>, <code>Toolbar</code>,
