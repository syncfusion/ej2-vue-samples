...
<div class="col-lg-12 control-section format">
    <div class='content-wrapper'>
        <div class='sliderwrap'>
            ...
            <ejs-slider id='currency' :value='value' :min='min' :max='max' :tooltip='tooltip' :ticks='ticks' :type='rangetype'></ejs-slider>
        </div>
        <div class='sliderwrap format'>
            <label class='labeltext'>Kilometer Slider</label>
            <ejs-slider id='kilometer' :value='kilovalue' :type='rangetype' :min='minkilo' :max='maxkilo' :tooltip='kilotooltip'
                :ticks='kiloticks'></ejs-slider>
        </div>
        <div class='sliderwrap'>
            <label class='labeltext'>Time Slider</label>
            <ejs-slider id='time' :value='timevalue' :type='rangetype' :min='mintime' :step='steptime' :max='maxtime' :tooltip='timetooltip'
                :ticks='timeticks' :tooltipChange='tooltipChangeHandler' :renderingTicks='renderingTicksHandler'></ejs-slider>
        </div>
    </div>
...
<div id="description">
    <p>The format feature customize the units of Slider values to desired format. The formatted values will also be applied
        to the ARIA attributes of the slider</p>
    ...
    <p>For more information, refer to the
        <a target="_blank" href="http://ej2.syncfusion.com/vue/documentation/slider/format.html">format</a> section from the documentation.</p>
...
export default Vue.extend({
  data: function() {
    return {
     ...
     steptime: 3600000 / 6,
     timevalue: [new Date(2013, 6, 13, 12).getTime(), new Date(2013, 6, 13, 18).getTime()],
     timetooltip: {
        placement: 'Before',
        isVisible: true,
    },
 ...
 methods : {
    ...
    onScroll: function(){
        if(!isNullOrUndefined(document.getElementById('currency'))){
        let currencyObj = document.getElementById('currency').ej2_instances[0];
        let kilometerObj = document.getElementById('kilometer').ej2_instances[0];
        let timeObj = document.getElementById('time').ej2_instances[0];
        let slider = [currencyObj, kilometerObj, timeObj];
