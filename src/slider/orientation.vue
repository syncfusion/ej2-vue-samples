    ...
    <div class='content-wrapper orientation'>
       <table class="slider_table">
        ...
        <div class='sliderwrap'>
            <ejs-slider id='default' :value='value' :orientation='orientation'></ejs-slider>
        </div>
                </td> <td>
        <div class='sliderwrap'>
            <ejs-slider id='minrange' :value='value' :type='mintype' :orientation='orientation'></ejs-slider>
        </div>
        </td> <td>
        <div class='sliderwrap'>
            <ejs-slider id='range' :value='rangevalue' :type='rangetype' :orientation='orientation'></ejs-slider>
        </div>
        </td> </tr>
            </tbody></table>
    </div>
...
<div id="description">
    <p>The Slider component allows to select a value or range of values in-between a min and max range, by dragging the thumb
        over the slider bar in vertical orientation. Three types of sliders in vertical orientation are: </p>
    <ul>
        ...
        <li>Range – allows to select a range of values with two handles, where the handles are connected with a range selection
        ...
        <p>For more information, refer to the
            <a target="_blank" href="http://ej2.syncfusion.com/vue/documentation/slider/getting-started.html#orientation">orientation</a> section from the documentation.</p>
...
export default Vue.extend({
 ...
 methods : {
      enableDisableTicks: function(args) {
        let defaultObj = document.getElementById('default').ej2_instances[0];
        let minRangeObj = document.getElementById('minrange').ej2_instances[0];
        let rangeObj = document.getElementById('range').ej2_instances[0];
        let ticks= { placement: args.checked ? 'Before' : 'None', largeStep: 20, smallStep: 5, showSmallTicks: true };
        defaultObj.ticks = ticks;
        minRangeObj.ticks = ticks;
        rangeObj.ticks = ticks;
    },
    enableDisableTooltip: function(args) {
        ...
        defaultObj.tooltip = tooltip;
        minRangeObj.tooltip = tooltip;
        rangeObj.tooltip = tooltip;
    },
    onScroll: function(){
        if(!isNullOrUndefined(document.getElementById('default'))){
        let defaultObj = document.getElementById('default').ej2_instances[0];
        let minRangeObj = document.getElementById('minrange').ej2_instances[0];
        let rangeObj = document.getElementById('range').ej2_instances[0];
        let slider = [defaultObj, minRangeObj, rangeObj];
